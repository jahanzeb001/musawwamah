...
class CalendarClient {
  ...
  insert(title, startTime, endTime) {
    var _clientID = new ClientId(
        "333799114774-da6tsjh25l2cdri4hq35a0ika9gkppis.apps.googleusercontent.com",
        "");
    clientViaUserConsent(_clientID, _scopes, prompt(title, startTime, endTime))
  ...
  prompt(title, startTime, endTime) async {
    // ignore: deprecated_member_use
    if (await canLaunch(
        'http://www.google.com/calendar/event?action=TEMPLATE&dates=$startTime%2F$endTime&text= $title')) {
      // ignore: deprecated_member_use
      await launch(
          'http://www.google.com/calendar/event?action=TEMPLATE&dates=$startTime%2F$endTime&text= $title');
    } else {
...
//                 DateTime sdate = intl.DateFormat.jm().parse(start);
//                 DateTime edate = intl.DateFormat.jm().parse(end);
//                 final dateFormat = intl.DateFormat.yMd().parse(adate);
//                 var actualdate = intl.DateFormat("yyyyddMM").format(dateFormat);

//                 var actualstime = intl.DateFormat("HH:mmss").format(sdate);
//                 var actual1 = actualstime.toString().replaceAll(':', '');

//                 var actualetime = intl.DateFormat("HH:mmss").format(edate);
//                 var actual2 = actualetime.toString().replaceAll(':', '');
//                 var conformsdate = '${actualdate}T${actual1}0ZUTC+05:00';
//                 var conformedate = '${actualdate}T${actual2}0ZUTC+05:00';

//                 print('${actualdate}T${actualstime}0ZUTC+03:00');
//                 print('${actualdate}T${actualetime}0ZUTC+03:00');
//                 CalendarClient()
//                     .insert(title, conformsdate, conformedate, slocation);
//               },